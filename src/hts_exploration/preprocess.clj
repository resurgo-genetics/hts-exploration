...
(defn fastq->table [round-number infile outfile]
  (let [r round-number
        c (atom 0)]
   (io/with-out-writer outfile
     ...
     (doseq [[nm inseq _ qual] (->> infile
       ...
       (let [[from to] (cond (= strand 1) (str-regex-index forward-re inseq)
         ...
         (swap! c inc)
         (->> (apply prn-str [@c nm inseq qual qfilter std-bases
...
(comment
  ...
  "load data local INFILE '/home/kitia/TMP/s15-round11-mysql.table' into table selex_test FIELDS TERMINATED by ' ' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 ROWS;"
  ...
  (let [stmts (map (fn [part]
    ...
    (doseq [stmt stmts]
      (clojure.java.jdbc/execute! mysql-test/mysql-ds [stmt])));update table
